...
int main(int argc, char ** argv)
{
  ...
  bool is_multi(false);
  std::vector<std::shared_ptr<ros_beginner::Node2Go>> node_vector;
  ...
  if (argc >= 3) {
    ...
    if (multi_flag == std::string("m")) {
      ...
      std::cout << "Example: ros2 run ch3_node_cpp getid <node_count> s/m" << std::endl;
      return 0;
    }
  } else {
    std::cout << "Example: ros2 run ch3_node_cpp getid <node_count> s/m" << std::endl;
  ...
  for (int i = node_count; i--; ) {
    node_vector.push_back(
      std::make_shared<ros_beginner::Node2Go>(
        "cpp_node_a_" +
        std::to_string(i)));
