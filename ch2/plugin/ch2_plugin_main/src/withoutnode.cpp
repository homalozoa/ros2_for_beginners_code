...
int main(int argc, char ** argv)
{
  ...
  pluginlib::ClassLoader<ch2::plugin::PluginBase> loader(
    "ch2_plugin_base",
    "ch2::plugin::PluginBase");
  // for (const auto & xml_path : loader.getPluginXmlPaths()) {
  //   std::cout << "xml path : " << xml_path << std::endl;
  // }
  // if (loader.isClassAvailable("ch2::plugin::PluginAlphaA")) {
  //   std::cout << "Ready to load 'ch2::plugin::PluginAlphaA'" << std::endl;
  //   loader.loadLibraryForClass("ch2::plugin::PluginAlphaA");
  // }
  // std::cout << "Load plugin "
  //   << loader.isClassAvailable("ch2::plugin::PluginAlphaA") << std::endl;

  try {
    auto alpha_a = loader.createUniqueInstance("ch2::plugin::PluginAlphaA");
  ...
  try {
    auto gamma = loader.createUniqueInstance("ch2::plugin::PluginGamma");
    gamma->say_hello(1);
  } catch (pluginlib::PluginlibException & ex) {
