...
#include "ch2_node_cpp/lifecyclenode2go.hpp"
...
int main(int argc, char ** argv)
{
  ...
  if (argc >= 3) {
    ...
    if (multi_flag == std::string("m")) {
      ...
      std::cout << "Example: ros2 run ch2_node_cpp multilifecyclenode <node_count> s/m" <<
        std::endl;
      return 0;
    }
  } else {
    std::cout << "Example: ros2 run ch2_node_cpp multilifecyclenode <node_count> s/m" << std::endl;
    return 0;
  }
